...
from core.trainer import KnowledgeAcquisition, do_KA_train
...
def main_worker(gpu, ngpus_per_node, args, final_output_dir):
    ...
    # student = eval('models.'+cfg.MODEL.NAME+'.get_pose_net')(cfg, is_train=True)
    # main = eval('models.'+cfg.MODEL.NAME+'.get_pose_net')(cfg, is_train=True)
    # comp = eval('models.'+'hrnet_comp'+'.get_pose_net')(cfg, is_train=True)

    train_loader = make_dataloader(
        cfg, is_train=True, distributed=args.distributed
    )

    model = KnowledgeAcquisition(cfg)
    if args.distributed:
    ...
    optimizer = get_optimizer(cfg, model.module.student)
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, cfg.TRAIN.LR_STEP, cfg.TRAIN.LR_FACTOR,
        last_epoch=-1
    )
    loss_factory = loss_main(cfg).cuda()
    gpu = args.gpu
    if gpu == 0:
        logger.info('Init KA training')
    ...
    for epoch in range(epochs):
        ...
        do_KA_train(cfg, gpu, model, train_loader, optimizer, loss_factory, epoch)

        lr_scheduler.step()
        if not cfg.MULTIPROCESSING_DISTRIBUTED or (
                cfg.MULTIPROCESSING_DISTRIBUTED
                and args.rank == 0) and (epoch % 25 == 0) :
    ...
    torch.save(model.module.student.state_dict(), final_model_state_file)
...
if __name__ == "__main__":
    ...
    print('Visible GPUs: {}'.format(os.environ['CUDA_VISIBLE_DEVICES']))
