...
from core.loss_comp import MultiLossFactory as loss_comp
from core.trainer import do_PT_train
...
def main_worker(gpu, ngpus_per_node, args, final_output_dir):
    ...
    if 'main' in cfg.MODEL.NAME:
        ...
        loss_factory = loss_comp(cfg).cuda()
    ...
    gpu = args.gpu
    if gpu == 0:
        logger.info('Init PT training')

    epochs = cfg.TRAIN.END_EPOCH
    for epoch in range(epochs):
        if args.distributed:
            train_loader.sampler.set_epoch(epoch)
        do_PT_train(cfg, args.gpu, model, train_loader, loss_factory, optimizer, epoch)
        lr_scheduler.step()
