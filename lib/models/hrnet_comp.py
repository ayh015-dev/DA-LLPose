...
class PoseHigherResolutionNetComp(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(PoseHigherResolutionNetComp, self).__init__()
        ...
        config_tags = self.spec.HEAD_OFFSET
        self.num_joints = cfg.DATASET.NUM_JOINTS
        self.num_tags = self.num_joints 
        self.num_joints_with_center = self.num_joints
        self.offset_prekpt = config_tags['NUM_CHANNELS_PERKPT']
        
        tags_channels = self.num_tags*self.offset_prekpt
        self.transition_heatmap = self._make_transition_for_head(
                    inp_channels, config_heatmap['NUM_CHANNELS'])
        self.transition_tags = self._make_transition_for_head(
                    inp_channels, tags_channels)
        self.head_heatmap = self._make_heatmap_head(config_heatmap)
        self.tags_feature_layers, self.tags_final_layer = \
            self._make_separete_regression_head(config_tags)
    ...
    def forward(self, x):
        ...
        final_tags = []
        tags_feature = self.transition_tags(x)

        for j in range(self.num_joints):
            final_tags.append(
                self.tags_final_layer[j](
                    self.tags_feature_layers[j](
                        tags_feature[:,j*self.offset_prekpt:(j+1)*self.offset_prekpt])))

        tags = torch.cat(final_tags, dim=1)
        return heatmap, tags                  
