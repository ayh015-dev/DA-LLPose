...
def get_multi_stage_outputs(cfg, model, image, with_flip=False):
    ...
    if with_flip:
        if 'coco' in cfg.DATASET.DATASET:
            ...
            flip_index_heat = FLIP_CONFIG['EXLPOSE_WITH_CENTER']
            flip_index_offset = FLIP_CONFIG['EXLPOSE']
        else:
...
def get_high_confidence_heatmap(cfg, heatmap, thres=0.9):
    maxm = hierarchical_pool(cfg, heatmap)
    maxm = torch.eq(maxm, heatmap).float()
    heatmap = heatmap * maxm
    scores = heatmap.view(-1)
    scores, pos_ind = scores.topk(cfg.TRAIN.MAX_NUM_CENTERS)

    threshold = cfg.TRAIN.TEACHER_THRESHOLD 
    select_ind = (scores > threshold).nonzero()
    scores = scores[select_ind][:, 0]
    pos_ind = pos_ind[select_ind][:, 0]
