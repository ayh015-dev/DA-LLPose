...
class Compose(object):
    ...
    def __call__(self, image, mask, oob_mask, joints, area, image_aug=None):
        for t in self.transforms:
            image, mask, oob_mask, joints, area, image_aug = t(image, mask, oob_mask, joints, area, image_aug=image_aug)
        return image, mask, oob_mask, joints, area, image_aug
...
class ToTensor(object):
    def __call__(self, image, mask, oob_mask, joints, area, image_aug=None):
        return F.to_tensor(image), mask, oob_mask, joints, area, F.to_tensor(image_aug) if image_aug is not None else None
...
class Normalize(object):
    ...
    def __call__(self, image, mask, oob_mask, joints, area, image_aug=None):
        ...
        if image_aug is not None:
            image_aug = F.normalize(image_aug, mean=self.mean, std=self.std)
        return image, mask, oob_mask, joints, area, image_aug
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, mask, oob_mask, joints, area, image_aug=None):
        ...
        if random.random() < self.prob:
            ...
            oob_mask = oob_mask[:, ::-1] - np.zeros_like(oob_mask)

            if image_aug is not None:
                image_aug = image_aug[:, ::-1] - np.zeros_like(image_aug)
...
class RandomAffineTransform(object):
    ...
    def __call__(self, image, mask, oob_mask, joints, area, image_aug=None):
        ...
        mat_output, _ = self._get_affine_matrix(
            center, scale, (self.output_size[0], self.output_size[0]), aug_rot
        )
        mat_output = mat_output[:2]
        oob_mask = cv2.warpAffine(
            oob_mask, mat_output, (self.output_size[0], self.output_size[0])
        )
        ...
        if image_aug is not None:
            image_aug = cv2.warpAffine(
                image_aug, mat_input, (self.input_size, self.input_size)
            )
